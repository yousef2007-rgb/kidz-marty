            ...
            <div className='py-7 flex items-center justify-center px-3 bg-secondary-color flex items-center justify-between'>
                <div className='ml-auto max-w-lg flex-1 relative mr-2'>
                    <div className='flex bg-secondary w-full p-2 rounded-md'>
                        <input className='flex-1 w-full outline-none bg-secondary' type='text' placeholder='search' onChange={handleInputChange} value={inputFieldValue} />
                        <button onClick={handleClearButton}>{inputFieldValue == '' ? <SearchIcon /> : <CloseIcon />}</button>
                    </div>
                    <div className='bg-secondary rounded-md max-h-[70vh] overflow-auto absolute w-full mt-2 flex flex-col top-full'>
                        {products.filter(searchFilter).map((product: Product, index: number) => (
                            <Link className="flex p-2 rounded-md transition-all items-center hover:bg-gray-300" href={`/product/${product._id}`}>
                ...
                <button onClick={handleClose} className='ml-auto fill-white hover:-translate-y-1 transition-all'><CloseIcon /></button>
            </div>
        </div>
    )
}

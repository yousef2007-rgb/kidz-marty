...
const page = async ({ params }: { params: { id: string, varient: string } }) => {
    const productData: Product = await getProduct(params.id);
    const varient: Varient | undefined =
        productData.varients?.find((varient, index) => varient._id == params.varient);
    console.log(varient)
    return (
        <main className='flex sm:flex-row flex-col flex-1 relative w-full'>
            <ImageSlider varient={varient} productData={productData} />
            <article className='sm:w-1/2 my-5 w-full px-5'>
                <h1 className='font-medium mb-4 text-3xl'>{varient?.title}</h1>
                ...
                <p className='my-4'>{varient?.discription}</p>
                ...
                <div className='flex flex-wrap'>
                    <Link href={`/product/${productData._id}`} className='max-w-[150px] bg-white flex rounded-md flex-1 mr-2'><img loading='lazy' className=' aspect-square rounded-md object-contain w-full' src={`${process.env.URL}/${productData.imageUrl}`} alt={productData.title} /></Link>
                    {productData.varients ? productData.varients.filter((varient) => varient._id != params.varient).map((varient, index) => (
                        <Link href={`/product/${productData._id}/varient/${varient._id}`} className='max-w-[150px] flex rounded-md flex-1 mr-2 bg-white' key={index}><img loading='lazy' className=' aspect-square rounded-md object-contain w-full' src={`${process.env.URL}/${varient.imageUrl}`} alt={varient.title} /></Link>
                    )) : ""}
                </div>
            </article>
        </main >

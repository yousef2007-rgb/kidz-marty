...
const page = (props: {}) => {
  ...
  const handleCheckout = async () => {
    ...
    if (token) {
      try {
        ...
        router.push(`/thankyoupage/?data=${JSON.stringify(response.data)}`);
      } catch (err: any) {
  ...
  return (
    <div className="container mx-auto px-5 flex-1  flex flex-col justify-center">
      {cartItems.length === 0 ? (
        <div className="text-center mt-10 font-bold text-xl capitalize flex min-h-full items-center">
          ...
          <div className="mt-10">
            <div className="flex items-center sm:flex-row flex-col">
              <button
                ...
                className="bg-secondary-color border-2 w-full sm:w-fit border-secondary-color hover:bg-transparent hover:text-secondary-color font-semibold text-white rounded-lg px-4 py-2"
              >
                Checkout
              </button>
